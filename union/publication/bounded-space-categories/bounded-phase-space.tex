...
\hypersetup{
    ...
    citecolor=blue,
    urlcolor=blue,
    pdftitle={On the Thermodynamic Consequences of Bounded Phase Space: Trajectory Completion Dynamics for Geometric Partitioning Coordinates},
...
% Title
\title{\textbf{On the Thermodynamic Consequences of Bounded Phase Space: Trajectory Completion Dynamics for Geometric Partitioning Coordinates}}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/panel_sentropy.png}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/panel_chromatography.png}
...
\subsubsection{Salt Adduct Validation via Ionic Conductivity}

A critical validation of the partition coordinate framework uses salt contamination naturally present in proteomics and lipidomics extraction protocols. Salts (Na$^+$, K$^+$, Li$^+$) form adducts with analyte molecules, appearing as characteristic mass shifts in mass spectra.

\begin{theorem}[Ionic Conductivity-Partition Relationship]
\label{thm:ionic_conductivity}
Ionic conductivity $\sigma$ scales with partition capacity $C(n) = 2n^2$:
\begin{equation}
    \sigma \propto C(n) \cdot \exp\left(-\frac{E_a}{RT}\right)
\end{equation}
where $E_a$ is the activation energy for ionic mobility and $n$ is the principal partition quantum number of the conducting ion.
\end{theorem}

\begin{proof}
Conductivity requires charge carriers to traverse partition boundaries. The number of available states for this traversal is $C(n) = 2n^2$. The probability of boundary crossing follows Boltzmann statistics with activation energy $E_a$. The product gives the conductivity scaling.
\end{proof}

Salt adduct detection in mass spectra validates this framework:
\begin{enumerate}
    \item Na$^+$ adducts: $\Delta m/z = 21.982$ (Na - H)
    \item K$^+$ adducts: $\Delta m/z = 37.956$ (K - H)
    \item Detection confirms partition coordinate assignment
\end{enumerate}

\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/panel_salt_conductivity.pdf}
\caption{\textbf{Salt Adduct Validation via Ionic Conductivity.} Salt contamination in proteomics/lipidomics extractions provides validation of the partition coordinate framework through ionic conductivity relationships. \textbf{Panel (a):} 3D conductivity surface showing $\sigma(C, T)$ dependence on concentration $C$ and temperature $T$. The surface follows $\sigma = \sigma_0 \sqrt{C} \exp(-E_a/RT)$, consistent with partition-mediated charge transport. \textbf{Panel (b):} Salt adduct intensity ratios from real mzML data. Na$^+$ (blue) and K$^+$ (purple) adduct peaks are detected at characteristic mass shifts from [M+H]$^+$ ions. The adduct-to-protonated ratio reflects partition state accessibility. \textbf{Panel (c):} Partition capacity $C(n) = 2n^2$ (circles) versus scaled ionic conductivity (squares) for elements with principal quantum numbers $n = 1$--7. Experimental conductivities (stars) for Na, K, Li, and Ca salts validate the linear relationship. \textbf{Panel (d):} Shell filling fraction for elements H--Mg showing bond completion tendency. Elements with filling $> 0.5$ (green) readily form ionic compounds; those below (orange) require covalent bonding. This validates the Bond Completion Theorem: chemical bonds complete partition shells.}
\label{fig:panel_salt_conductivity}
\end{figure*}

The detection of Na$^+$ and K$^+$ adducts with correct mass shifts validates that partition coordinates correctly assign ionic states, and that conductivity scales with partition capacity as predicted.
