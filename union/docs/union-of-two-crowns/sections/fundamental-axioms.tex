...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/phase_space_partitions.png}
    \caption{Phase Space Partition Structure Across Five Thermodynamic Regimes. 
    3D visualizations show partition depth $n$ distributions in neutral gas, plasma, degenerate matter, relativistic gas, and Bose-Einstein condensate states. Spatial scales range from microscopic (0.04 \textmu m, degenerate matter) to macroscopic ($10^4$ \textmu m, neutral gas/plasma). Partition depth probability distributions (bottom right) reveal regime-specific statistics: classical regimes exhibit exponential decay from $n = 1$, quantum regimes show suppressed high-$n$ states, relativistic regime displays intermediate behavior. All distributions satisfy capacity theorem $C(n) = 2n^2$ with $>99.9\%$ agreement.}
