...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/figure_3_capacity_formula.png}
    \caption{\textbf{Partition capacity scaling and state counting validation.}
    (\textbf{A}) Capacity versus partition depth showing $C(n) = 2n^2$ scaling law. Blue line with circles demonstrates quadratic growth from $C(1) = 2$ to $C(10) = 200$. Data points confirm exact agreement with theoretical prediction across all partition levels.
    (\textbf{B}) Geometric derivation of capacity formula. Red circles show radial states ($n$), green diamonds show angular states ($2n$), blue circles show total states ($n + 2n = 2n^2$). Demonstrates how geometric partition structure naturally generates the $2n^2$ degeneracy pattern.
    (\textbf{C}) Quantum mechanical validation using hydrogen-like atoms. Purple line shows quantum calculation $\sum_{\ell=0}^{n-1} 2(2\ell+1) = 2n^2$ with experimental data points (purple stars). Dashed line shows theoretical $C(n) = 2n^2$ for comparison, confirming perfect agreement.
    (\textbf{D}) Classical phase space cell counting. Cyan line with squares shows accessible phase space cells versus energy level index $n$. Dashed line indicates $C(n) = 2n^2$ theoretical scaling. Demonstrates equivalence between quantum state counting and classical phase space partitioning.}
