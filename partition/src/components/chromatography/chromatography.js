...
export default function Chromatography({ ActiveIndex }) {
  ...
  useEffect(() => {
    ...
    const gradient = defs.append('linearGradient')
      .attr('id', 'tic-gradient').attr('x1', '0%').attr('y1', '0%').attr('x2', '0%').attr('y2', '100%')
    ...
    g.append('path').datum(data)
      .attr('d', d3.area().x(d => x(d.time)).y0(h).y1(d => y(d.intensity)).curve(d3.curveBasis))
      .attr('fill', 'url(#tic-gradient)')

    g.append('path').datum(data)
      .attr('d', d3.line().x(d => x(d.time)).y(d => y(d.intensity)).curve(d3.curveBasis))
      .attr('fill', 'none').attr('stroke', '#9980FA').attr('stroke-width', 2)
    ...
    g.append('text').attr('x', w / 2).attr('y', h + 35)
      .attr('text-anchor', 'middle').attr('fill', '#b0b0b0').attr('font-size', 11).text('Retention Time (min)')
    g.append('text').attr('transform', 'rotate(-90)').attr('x', -h / 2).attr('y', -38)
      .attr('text-anchor', 'middle').attr('fill', '#b0b0b0').attr('font-size', 11).text('Relative Intensity')
  }, [ActiveIndex])

  // Chart 2: S-Entropy Projection
  useEffect(() => {
    if (ActiveIndex !== 3) return
    ...
    g.selectAll('circle').data(data).enter().append('circle')
      .attr('cx', d => x(d.sk)).attr('cy', d => y(d.st)).attr('r', d => rScale(d.se))
      .attr('fill', '#f9d77e').attr('fill-opacity', 0.5)
      .attr('stroke', '#f9d77e').attr('stroke-opacity', 0.8).attr('stroke-width', 1)

    g.append('g').attr('transform', `translate(0,${h})`).call(d3.axisBottom(x).ticks(5))
      .selectAll('text').attr('fill', '#b0b0b0')
    g.selectAll('.domain, .tick line').attr('stroke', '#333333')

    g.append('g').call(d3.axisLeft(y).ticks(5)).selectAll('text').attr('fill', '#b0b0b0')
    g.selectAll('.domain, .tick line').attr('stroke', '#333333')

    g.append('text').attr('x', w / 2).attr('y', h + 35)
      .attr('text-anchor', 'middle').attr('fill', '#b0b0b0').attr('font-size', 11).text('Sk (kinetic)')
    g.append('text').attr('transform', 'rotate(-90)').attr('x', -h / 2).attr('y', -38)
      .attr('text-anchor', 'middle').attr('fill', '#b0b0b0').attr('font-size', 11).text('St (thermodynamic)')
  }, [ActiveIndex])
        ...
        <div className="scrolly-section" ref={sectionRef}>
          ...
          <div className="chart-wrapper">
            <div className="chart-card">
              ...
              <svg ref={chromatogramRef} viewBox="0 0 480 280" style={{ width: '100%', height: 'auto' }} />
            </div>
            <div className="chart-card">
              <div className="chart-title">S-Entropy Projection (Sk vs St)</div>
              <svg ref={entropyRef} viewBox="0 0 480 280" style={{ width: '100%', height: 'auto' }} />
            </div>
          </div>
          ...
          <div className="scroll-steps">
            <section className="step step-header">
              <span className="step-label">Chromatography</span>
