...
export default function Metabolomics({ ActiveIndex }) {
  ...
  useEffect(() => {
    ...
    const x = d3.scaleLinear().domain([0, 110]).range([0, innerWidth])
    const y = d3.scaleBand().domain(data.map(d => d.level)).range([0, innerHeight]).padding(0.4)
    ...
    g.append('g').call(d3.axisLeft(y))
      .selectAll('text').attr('fill', '#b0b0b0').style('font-size', '12px')
    g.selectAll('.domain, line').attr('stroke', '#333333')

    g.selectAll('.bar').data(data).enter().append('rect')
      .attr('class', 'bar').attr('y', d => y(d.level)).attr('height', y.bandwidth())
      .attr('x', 0).attr('width', 0).attr('fill', d => colors[d.level]).attr('rx', 3)
      .transition().duration(800).attr('width', d => x(d.count))
    ...
    g.selectAll('.bar-label').data(data).enter().append('text')
      ...
      .attr('fill', '#b0b0b0').style('font-size', '12px').text(d => d.count)
  ...
  useEffect(() => {
    ...
    const y = d3.scaleLinear().domain([0, 1250]).range([innerHeight, 0])
    ...
    g.selectAll('circle').data(points).enter().append('circle')
      .attr('cx', d => x(d.rt)).attr('cy', d => y(d.mz))
      .attr('r', 0).attr('fill', '#9980FA').attr('opacity', 0.6)
      .transition().duration(600).delay((_, i) => i * 30).attr('r', 4)
        ...
        <div className="scrolly-section" ref={sectionRef}>
          ...
          <div className="scroll-steps">
            <section className="step step-header">
              <span className="step-label">Metabolomics</span>
