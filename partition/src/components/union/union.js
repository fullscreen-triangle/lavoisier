...
export default function Union({ ActiveIndex }) {
  ...
  useEffect(() => {
    ...
    g.append('circle').attr('r', 0).attr('fill', '#f9d77e').attr('opacity', 0.9)
      .transition().duration(600).attr('r', centerRadius)

    g.append('text').attr('text-anchor', 'middle').attr('dy', '-0.3em')
      .attr('fill', '#1a1a1a').style('font-size', '9px').style('font-weight', 'bold').text('Bounded')
    g.append('text').attr('text-anchor', 'middle').attr('dy', '0.7em')
      .attr('fill', '#1a1a1a').style('font-size', '9px').style('font-weight', 'bold').text('Phase Space')

    const ring1Data = ['Oscillation', 'Partitions', '3D Space', 'Charge', 'Mass']
    const ring1Inner = 55, ring1Outer = 100
    const ring1Arc = d3.arc().innerRadius(ring1Inner).outerRadius(ring1Outer)
    const ring1Angle = (2 * Math.PI) / ring1Data.length, ring1Pad = 0.04

    ring1Data.forEach((label, i) => {
      const startAngle = i * ring1Angle + ring1Pad
      const endAngle = (i + 1) * ring1Angle - ring1Pad

      g.append('path').attr('d', ring1Arc({ startAngle, endAngle }))
        .attr('fill', '#9980FA').attr('opacity', 0).attr('stroke', '#1a1a1a').attr('stroke-width', 1)
        .transition().duration(600).delay(300 + i * 80).attr('opacity', 0.85)

      const midAngle = (startAngle + endAngle) / 2 - Math.PI / 2
      const labelR = (ring1Inner + ring1Outer) / 2
      g.append('text').attr('x', Math.cos(midAngle) * labelR).attr('y', Math.sin(midAngle) * labelR)
        .attr('text-anchor', 'middle').attr('dy', '0.35em')
        .attr('fill', '#ffffff').style('font-size', '9px').style('font-weight', '600').text(label)
    })

    const ring2Data = [
      'E = mc\u00B2', 'Equivalence', 'Lorentz Inv.', 'Selection Rules',
      'E = \u0127\u03C9', 'Partition Coords', 'Quantum Nos.', '\u0394l = \u00B11',
      'Lagrangian', 'Landauer Limit', 'Bijective Map', 'Ion Journey'
    ]
    const ring2Inner = 108, ring2Outer = 155
    const ring2Arc = d3.arc().innerRadius(ring2Inner).outerRadius(ring2Outer)
    const ring2Angle = (2 * Math.PI) / ring2Data.length, ring2Pad = 0.03
    ...
    ring2Data.forEach((label, i) => {
      ...
      g.append('path').attr('d', ring2Arc({ startAngle, endAngle }))
        .attr('fill', '#9980FA').attr('opacity', 0).attr('stroke', '#1a1a1a').attr('stroke-width', 1)
        .transition().duration(600).delay(700 + i * 60).attr('opacity', opacity)

      const midAngle = (startAngle + endAngle) / 2 - Math.PI / 2
      const labelR = (ring2Inner + ring2Outer) / 2
      g.append('text').attr('x', Math.cos(midAngle) * labelR).attr('y', Math.sin(midAngle) * labelR)
        .attr('text-anchor', 'middle').attr('dy', '0.35em')
        .attr('fill', '#e0e0e0').style('font-size', '7.5px').text(label)
    })
  ...
  useEffect(() => {
    ...
    const x = d3.scaleLog().domain([10, 200000]).range([0, innerWidth])
    const y = d3.scaleBand().domain(data.map(d => d.category)).range([0, innerHeight]).padding(0.35)

    const g = svg.append('g').attr('transform', `translate(${margin.left},${margin.top})`)

    g.append('g').attr('transform', `translate(0,${innerHeight})`)
      .call(d3.axisBottom(x).ticks(4, '~s')).selectAll('text').attr('fill', '#b0b0b0').style('font-size', '11px')
    g.selectAll('.domain, line').attr('stroke', '#333333')

    g.append('g').call(d3.axisLeft(y)).selectAll('text').attr('fill', '#b0b0b0').style('font-size', '10px')
    g.selectAll('.domain, line').attr('stroke', '#333333')

    g.selectAll('.bar').data(data).enter().append('rect')
      .attr('class', 'bar').attr('y', d => y(d.category)).attr('height', y.bandwidth())
      .attr('x', 0).attr('width', 0).attr('fill', '#9980FA')
      .attr('opacity', (_, i) => 0.55 + i * 0.1).attr('rx', 3)
      .transition().duration(800).delay((_, i) => i * 120).attr('width', d => x(d.count))

    g.selectAll('.bar-label').data(data).enter().append('text')
      .attr('class', 'bar-label').attr('x', d => x(d.count) + 8)
      .attr('y', d => y(d.category) + y.bandwidth() / 2 + 4)
      .attr('fill', '#b0b0b0').style('font-size', '11px').text(d => d.label)
        ...
        <div className="scrolly-section" ref={sectionRef}>
          ...
          <div className="scroll-steps">
            <section className="step step-header">
              <span className="step-label">Union</span>
