...
class PeptideSummary:
    ...
    def load_results(self, file_name):
        ...
        for line in open_file:
            line = '\t'.join(
                [x.strip() if x.strip() else ' ' for x in line.split('\t')])  # remove end-of-line characters
        ...
        if redo_split:  # this is only True if something got combined or grouped
            ...
            for i, acc in enumerate(self.acc_list):
                if '&' in acc:
                    ...
                    self.proteins.tuplets[
                        tuplet_index].correcteds = new_corr_counts  # replace old corrected counts with new ones
        return
    ...
    def test_redundants(self, number):
        ...
        for i in range(len(self.acc_list)):
            ...
            for j in range(i + 1, len(self.acc_list)):
                ...
                share_count = self.total_count_set(share_set, self.tot_count[
                    j])  # total count of shared peptides (can use either i or j)
                ...
                unique_j_count = self.total_count_set(unique_j_set,
                                                      self.tot_count[j])  # total count of "right" peptides
    ...
    def test_subsets(self, number):
        ...
        for i in range(len(self.acc_list)):
            ...
            for j in range(i + 1, len(self.acc_list)):
                ...
                share_count = self.total_count_set(share_set,
                                                   self.tot_count[j])  # total shared counts (could use either i or j)
                ...
                unique_j_count = self.total_count_set(unique_j_set,
                                                      self.tot_count[j])  # total counts of "right" uniques
                ...
                if (share_count >= 10 * max(unique_i_count, 1)) and (
                        unique_i_count <= self.pseudo):  # don't care about "right" uniques
    ...
    def test_shares(self, number):
        ...
        for i in range(len(self.acc_list)):
            ...
            for j in range(i + 1, len(self.acc_list)):
                ...
                if unique_i_count > 0:
                    share_i_mass = share_count / float(
                        unique_i_count)  # relative weight (mass) of shared to uniques for "left"
                else:
                    share_i_mass = 1000.0  # something big instead of divide by zero
                if unique_j_count > 0:
                    share_j_mass = share_count / float(
                        unique_j_count)  # relative weight (mass) of shared to uniques for "right"
                else:
