...
from lipidomics.SpectraReader import extract_spectra


def extract_zip_files(folder_path: str):
...
def extraction(file, save_folder):
    filename_base = os.path.split(file)[1]
    filename_base = filename_base.replace(".mzML", "")
    file_header = filename_base + 'ms1_xic_df' + '.tab'
    exp_date, samplename, polarity, repNumber, treatment, lctype = extract_metadata(file)
    _, _, ms1_xic_df = extract_spectra(file, [5, 20], 6, 1000, 10, 50e-6, 500e-6, 3, "thermo")
    ms1_xic_df['exp_date'] = exp_date
    ms1_xic_df['samplename'] = str(samplename)
    ms1_xic_df['repNumber'] = repNumber
    ms1_xic_df['treatment'] = str(treatment)
    ms1_xic_df['lctype'] = str(lctype)
    if polarity == 'pos':
        ms1_xic_df['polarity'] = '+'
    else:
        ms1_xic_df['polarity'] = '-'

    path = os.path.join(save_folder, file_header)
    path = str(path)
    ms1_xic_df.to_csv(path, index=False, encoding='utf-8', sep='\t')


def add_results_toLDataFrame(path: str, output_path):
    data = LDataFrame(path)
    timestamp = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
    file_header = output_path + timestamp
    path = os.path.join(output_path, file_header)
    path = str(path)
    data.to_csv(path, index=False, encoding='utf-8', sep='\t')


def extract_metadata(file_name):
    filename_base = os.path.split(file_name)[1]
    filename_base = filename_base.replace(".mzML", "")
    exp_date, samplename, polarity, repNumber, treatment, lctype = filename_base.split('_')
    return exp_date, samplename, polarity, repNumber, treatment, lctype
