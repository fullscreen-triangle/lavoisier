...
suppose one catalyst may be in error \emph{but which one is not known}.
Then $\amb(v)$ is stable under the removal of \emph{an} erroneous
catalyst only if $k\ge3$.
\end{proposition}
...
\begin{remark}[The hypothesis is doing the work]\label{rem:three-unaided}
The unidentifiability hypothesis is not decoration. If the erroneous
catalyst \emph{is} identified, deleting it by name leaves $k-1$ sound
catalysts, and one sound catalyst determines the region with no majority
required --- so $k=2$ suffices and the proposition is false. The two
readings differ at $k=2$ and nowhere else; measured recovery is
$0.000$ at $k=1$ under both, and $1.000$ at $k=2$ under the identified
reading against $0.000$ under the unaided one. The proposition as first
drafted said ``\emph{the} erroneous catalyst'', whose definite article
presupposes the identification the proof never assumes; the statement
above is the one the proof establishes.
\end{remark}
