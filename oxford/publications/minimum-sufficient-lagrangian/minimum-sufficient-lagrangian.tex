...
\begin{remark}[The Same $n$ Cycles Admit $T(n,d)$ Distinct Termination Points]
\label{rem:multiple_terminations}
Theorem~\ref{thm:composition} is standardly read as a counting result: $n$
oscillation cycles produce $T(n,d)$ distinguishable trajectories. But the
deeper implication concerns the structure of time composition itself.

It is not \emph{time} that inflates. The $n$ cycles occupy the same elapsed
duration regardless of how they are composed. What inflates is the
\emph{composition} of those $n$ cycles: any ordered partition
$(c_1, c_2, \ldots, c_k)$ with $c_1 + \cdots + c_k = n$ and each $c_i \geq
1$ describes a structurally distinct way of distributing the $n$-cycle period
across $k$ labeled partition steps. Each such composition is a different
termination point of the same $n$-step process.

Therefore: the same physical item --- the system after $n$ oscillation cycles
--- does not arrive at a single termination point in partition space. It
arrives at $T(n,d)$ distinct termination points, one for each labeled
composition of the $n$-step period. Which termination point is actualised
depends on the composition structure (the grouping and dimensional labeling of
the steps), not solely on their count. The minimum sufficient Lagrangian
$\Lmin$ corresponds to the ground-state composition (the unique composition
that achieves the contact floor $\bmin$); every other composition of the same
$n$ steps produces an inflated Lagrangian $\Lmin + \delta L$, where $\delta L
\geq 0$ measures the partition-depth deviation from the contact ground state.
The $T(n,d) - 1$ non-ground compositions are the Lagrangian inflations; the
single ground composition is the minimum.
\end{remark}
...
\begin{remark}[The Finite/Infinite Asymmetry in the Partition Framework]
\label{rem:finite_infinite}
The proof of Theorem~\ref{thm:loschmidt} relies on the strict positivity of the
partition floor at every finite stage of the dynamics: $\bmin > 0$ holds for any
finite partition sequence, even one that in the limit $n \to \infty$ approaches
a dissolving trajectory ($\bmin \to 0$). The asymmetry is:

\begin{enumerate}[label=(\roman*)]
  \item In the limit ($n \to \infty$, the universe fully negated), the floor
  may approach zero for a dissolving partition component. But this limit is
  never achieved by any finite physical process. Every finite sequence of
  partition steps has a positive floor.

  \item The second law is a statement about every finite stage: $\delta\beta(t)
  \geq 0$ at each step $t$. It is not a statement about the infinite-time limit.
  Loschmidt's argument implicitly requires the infinite-time limit (completing
  the reversal sequence), which is outside the regime where $\bmin > 0$ is
  guaranteed.

  \item A partition component that dissolves in the limit is individuated at
  every finite stage: its separation cost $\sigma(v) > 0$ for all $t < \infty$.
  Only in the limit does individuation fail. Physical processes are finite.
\end{enumerate}

This finite/infinite asymmetry is the partition-theoretic counterpart of Poincaré
recurrence: recurrence times are finite for finite systems but grow unboundedly
with system size, making them physically inaccessible. Here, the dissolving
floor is similarly inaccessible in finite time. The second law holds because
we never reach the limit.
\end{remark}
...
\begin{remark}[Contact Potential Is Carried by the Medium Edge, Not the Analytical Edges]
\label{rem:medium_edge}
In the graph-theoretic representation of a mass spectrometric system, the
ion node $v$ has two structurally distinct types of incident edges:
the \emph{medium edge} $(\mathcal{M}, v)$ of weight $w(\mathcal{M}, v) = M_{\bmin}^{\mathrm{MS}}$
(the contact potential, encoding mass) and the \emph{analytical edges}
$(v, u_k)$ encoding fragmentation products, adducts, and co-eluting species
(inflation terms $\delta L$).

The contact potential $M_{\bmin}$ is carried \emph{exclusively} by the medium
edge. The analytical edges are partition depth inflations at depth $n_k$
above the contact floor; they contribute to the inflation term $\delta M$ in
$L = \Lmin + \delta L$, not to $M_{\bmin}$ itself. This is the graph-theoretic
content of the partition decomposition $L = \Lmin + \delta L$: the Lagrangian
splits according to which edges carry it.

A direct consequence is that tracking the minimum edge incident to $v$
(the node floor in graph-theoretic language) conflates $M_{\bmin}$ with
$\delta M$ if any analytical edge is lighter than the mass edge. The invariant
is the medium edge weight $w(\mathcal{M}, v)$ specifically --- preserved by
Theorem~\ref{thm:time_invariance} across all instrument generations --- not
the minimum of all incident edges. Any analytical property lighter than mass
is an inflation, not a floor.
\end{remark}
