...
The framework makes several falsifiable predictions testable against
existing Orbitrap and FT-ICR data.

\subsection{Primary prediction: fragment subharmonics}

\textbf{Prediction 1}: For any isolated precursor ion in a clean
Orbitrap measurement, the Fourier spectrum should contain peaks at
$\omega_{\text{prec}}\sqrt{m_{\text{prec}}/m_{f_i}}$ for all fragment
ions $f_i$ in the known fragmentation spectrum. The amplitude of each
subharmonic should be proportional to the abundance of the corresponding
fragment in the MS/MS spectrum.

\textit{Test protocol}: Take existing Orbitrap HCD data for a known
peptide. Apply extended Fourier transform to the MS1 transient (if
available) or repeat the experiment with longer MS1 transient time.
Look for peaks at the predicted subharmonic frequencies. The prediction
is that these peaks are present and correspond to the known fragment
masses.

\subsection{Charge state subharmonics}

\textbf{Prediction 2}: For a multi-charged ion $[\mathrm{M+2H}]^{2+}$
measured in the Orbitrap, the MS1 transient should contain a subharmonic
at $\omega_z^{(1)} = \omega_z^{(2)}/\sqrt{2}$ corresponding to the
singly-charged species, even if no $[\mathrm{M+H}]^+$ is present in
the trap. This is the charge-dimension virtual substate.

\textit{Test protocol}: Isolate a pure $[\mathrm{M+2H}]^{2+}$ ion in
the Orbitrap with no other charge states present. Acquire a long
($>500\,\mathrm{ms}$) transient. Fourier transform at high frequency
resolution. Look for a peak at $\omega_z^{(2)}/\sqrt{2}$.

\subsection{Polarity phase asymmetry}

\textbf{Prediction 3}: For a positive ion $[\mathrm{M+H}]^+$ in the
...
and $[\mathrm{M-H}]^-$. The phase asymmetry magnitude is
$\cos(\pi\Delta M_{\text{pol}})$ where $\Delta M_{\text{pol}} = M_{(+)} - M_{(-)}$.

\subsection{Initial state recovery}

\textbf{Prediction 4}: The ionization time of a precursor (relative to
the reference oscillator) should be recoverable from the current
oscillation phase via~\eqref{eq:initial-phase}. For ions created by
electrospray at a known spray voltage pulse, the recovered initial phase
should correspond to the partition state at the ionization moment.
