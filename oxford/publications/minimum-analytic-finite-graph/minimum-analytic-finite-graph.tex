...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{\textbf{On the Distinction of Partition Depth through Uncertainty Propagation:\\ Sufficient Contact Depth Boundaries in  Minimum Analytic Finite Weighted Graph}}

\author{Kundai Farai Sachikonye\\
\textit{Technical University of Munich}\\
\textit{AIMe Registry for Artifical Intelligence}\\
\textit{Maximus-von-Imhof-Forum 3, 85354 Freising, Germany}\\
...
node $\med \in \Vcal$ called the \emph{medium}, which is simultaneously in
contact with every other node; and the \emph{contact floor}
$\bmin > 0$, the minimum edge weight in $G$, proved strictly positive for
any finite graph with positive edge weights.

We prove four results. First, the \emph{Individuation Theorem}: a node $v
\in \Vcal$ is individuated --- distinct from the medium --- if and only if
every cut separating $v$ from the medium has weight $\geq \bmin$.
Individuation is a structural property of the graph, not a property of the
node in isolation. Second, the \emph{Truth Cell Theorem}: the truth of a
node $v$ is not a point in $\Vcal$ but the minimum cut set $C^*(v)$ of
weight exactly $\bmin$ separating $v$ from the medium --- a cell, not a
point. Third, the \emph{Reshuffling Theorem}: any measurement of $v$ is a
weight-preserving edge relabelling that produces a new arrangement of contact
edges without revealing identity; the sequence of measurements converges to
the contact floor $\bmin$, not to any node. Fourth, the
\emph{Invariance Theorem}: for any node $v$ present in an expanding sequence
of graphs $G_0 \subset G_1 \subset \cdots$ (the universe growing), the
intrinsic floor $\bmin^*(v) = \lim_{k\to\infty} \bmin(v, G_k)$ exists,
is strictly positive, and is independent of the size of the universe.
The intrinsic floor is the graph-theoretic definition of the identity of
$v$: the one thing about $v$ that does not change as more neighbours are
added. For $v$ to exist as a distinct thing, it must be finite (bounded
boundary cost) and its floor must be invariant (the same in every graph
containing $v$). These two requirements are the same requirement: both are
the statement $\bmin^*(v) > 0$.

We apply the framework to four cases: (i) population counting, where the
...
approaching identity; (iii) mass spectrometry specifically, where mass is
identified as the intrinsic floor $\bmin^*(v)$ --- the only property of an
ion that is invariant across all instrument generations, matrices, and
sample preparation histories; and (iv) the epistemology of classification,
where the sufficient truth of any classified object is the structured set of
graph-theoretic unknowables constituting its contact floor.
...
minimum cut; truth cell; graph reshuffling; medium; intrinsic floor; invariance;
population counting; analytical chemistry; mass spectrometry; mass as floor;
epistemology of measurement; graph automorphism; conservation
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item The \emph{Intrinsic Floor} $\bmin^*(v)$
  (Definition~\ref{def:intrinsic_floor}): the limit of the contact floor
  of $v$ as the surrounding graph grows without bound; the single
  graph-theoretic property of $v$ that is independent of the universe's
  size (Section~\ref{sec:invariance}).

  \item The \emph{Invariance Theorem} (Theorem~\ref{thm:invariance}): for
  any node $v$ in an expanding graph sequence, $\bmin^*(v) > 0$ and is
  stable; the two conditions for a thing to exist --- finitude and
  invariance under negation --- are the same condition
  (Section~\ref{sec:invariance}).

  \item Four applications: population counting, analytical chemistry,
  mass-as-floor in mass spectrometry, and classification epistemology
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Intrinsic Floor and the Invariance Theorem}
\label{sec:invariance}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Two Requirements for a Thing to Exist}

For a node $v$ to exist as a distinct thing --- not merely as a region of the
medium --- two conditions must hold simultaneously:

\begin{enumerate}[label=(\roman*)]
  \item \textbf{Finitude.} The boundary of $v$ must have finite cost. If the
  boundary were infinite, $v$ would be indistinguishable from the medium: it
  would have no finite cut separating it from everything else. An infinite
  boundary is not a boundary at all.

  \item \textbf{Invariance under negation.} The identity of $v$ must be stable
  as the negation process continues. If identifying $v$ required completing an
  infinite negation --- and if the result of that negation changed with each
  new neighbour added --- then $v$ would not be the same thing before and after
  the negation. Lithium identified today must be the same Lithium identified
  a century ago and a century hence, regardless of how many new elements have
  been catalogued in the interim.
\end{enumerate}

We now show these two conditions are the same condition.

\begin{definition}[Intrinsic Floor]
\label{def:intrinsic_floor}
Let $v$ be a node appearing in a sequence of finite weighted graphs $G_0
\subset G_1 \subset G_2 \subset \cdots$ where each $G_{k+1}$ is obtained
from $G_k$ by adding new nodes and edges (expanding the universe, adding more
neighbours). The \emph{intrinsic floor} of $v$ is:
\[
  \bmin^*(v) = \lim_{k \to \infty} \bmin(v, G_k)
\]
where $\bmin(v, G_k) = \min_{e \in \Ecal_k,\, e \text{ incident to } v} w(e)$
is the minimum edge weight incident to $v$ in $G_k$.
\end{definition}
...
\begin{lemma}[The Intrinsic Floor Exists]
\label{lem:intrinsic_exists}
For any node $v$ in an expanding graph sequence $G_0 \subset G_1 \subset \cdots$
with all edge weights strictly positive, the intrinsic floor $\bmin^*(v)$ exists
and satisfies $\bmin^*(v) \geq \bmin(G_0) > 0$.
\end{lemma}

\begin{proof}
The sequence $(\bmin(v, G_k))_{k \geq 0}$ is non-increasing: adding new
neighbours to $v$ can only introduce edges of weight $\leq$ existing minimum
weights or not. But all weights are strictly positive, so $\bmin(v, G_k) \geq
\bmin(G_0) > 0$ for all $k$ by Lemma~\ref{lem:floor_positive}. A
non-increasing sequence bounded below converges. The limit
$\bmin^*(v) \geq \bmin(G_0) > 0$ exists. \qed
\end{proof}

\begin{theorem}[Invariance Theorem]
\label{thm:invariance}
Let $v$ be a node in an expanding graph sequence $G_0 \subset G_1 \subset
\cdots$ where $|\Vcal_k| \to \infty$ (the universe grows without bound). Then:

\begin{enumerate}[label=(\roman*)]
  \item The intrinsic floor $\bmin^*(v) > 0$ is strictly positive regardless
  of the size of the universe.

  \item $\bmin^*(v)$ is independent of $|\Vcal_k|$: it does not change as
  more nodes are added to the graph. It is the same in a two-node universe
  as in an infinite universe.

  \item The two conditions for $v$ to exist as a distinct thing --- finitude
  (bounded boundary cost) and invariance under negation (stable identity
  through an incompletable NOT-process) --- are equivalent. Both are the
  single condition $\bmin^*(v) > 0$.

  \item If $v$ had a point identity --- a zero-cost boundary, $\bmin^*(v) = 0$
  --- then $v$ would be indistinguishable from the medium: it would be
  dissolved, not individuated.
\end{enumerate}
\end{theorem}

\begin{proof}
(i) By Lemma~\ref{lem:intrinsic_exists}, $\bmin^*(v) \geq \bmin(G_0) > 0$.

(ii) The intrinsic floor is the limit of a convergent sequence bounded below
by $\bmin(G_0) > 0$. Adding new nodes to $\Vcal_k$ adds new edges; these
edges have positive weight (Definition~\ref{def:fwg}), so they do not reduce
$\bmin(v, G_k)$ below the existing floor. The limit $\bmin^*(v)$ is therefore
determined by the graph structure local to $v$, not by the global size
$|\Vcal_k|$. In particular, $\bmin^*(v)$ computed in $G_0$ (two nodes) equals
$\bmin^*(v)$ computed in the limit (infinite nodes), since the edges incident
to $v$ in $G_0$ are present in all $G_k$.

(iii) Finitude requires $\bmin^*(v) < \infty$, which holds trivially for
any finite edge weight. The substantive content of finitude is
$\bmin^*(v) > 0$: the boundary exists and has positive cost. Invariance
under negation requires that the identity of $v$ --- whatever graph-theoretic
property of $v$ is stable across all $G_k$ --- is unchanged by adding more
nodes to $\Vcal_k$. By (ii), the unique such property is $\bmin^*(v)$: it
is the only quantity associated with $v$ that is independent of the universe's
size. Every other property of $v$ --- its neighbourhood $\Nbd_{G_k}(v)$, its
truth cell $C^*_{G_k}(v)$, its separation cost $\sigma_{G_k}(v)$ --- changes
as $k$ grows. Only $\bmin^*(v)$ is stable. Hence the condition for invariance
under negation is exactly $\bmin^*(v) > 0$: the same as finitude.

(iv) If $\bmin^*(v) = 0$, then for any $\varepsilon > 0$ there exists $G_k$
with an edge incident to $v$ of weight $< \varepsilon$. In the limit, $v$
can be separated from any neighbour at arbitrarily small cost --- it dissolves
into the medium. A dissolved node is not a distinct thing: it has no boundary
with positive cost. \qed
\end{proof}

\begin{corollary}[Identity Is the Floor]
\label{cor:identity_is_floor}
The identity of a node $v$ --- the graph-theoretic answer to ``what is $v$?''
--- is not its label, not its neighbourhood, not its truth cell. It is its
intrinsic floor $\bmin^*(v)$. The label is a convenience. The neighbourhood
changes with every reshuffling. The truth cell changes with every instrument
generation. Only the intrinsic floor is stable. The identity of $v$ is the
minimum cost of its boundary with the medium, in the limit as the universe
grows without bound.
\end{corollary}

\subsection{The Incompletable Negation}

\begin{proposition}[Negation Is Incompletable Iff the Universe Is Infinite]
\label{prop:incompletable}
The process of individuating $v$ by negation --- establishing what $v$ is not,
relative to each new neighbour added to the graph --- is completable if and
only if $|\Vcal|$ is finite. If the universe is infinite ($|\Vcal_k| \to
\infty$), the negation process never terminates. But the intrinsic floor
$\bmin^*(v)$ is defined and positive at every finite stage, and converges
to a positive limit. The thing $v$ is individuated at every finite stage
even though the negation is never complete.
\end{proposition}

\begin{proof}
At each stage $k$, the NOT-sequence of $v$ in $G_k$ is
$\{u \in \Vcal_k : u \neq v,\, (v,u) \in \Ecal_k\}$ --- the set of nodes
$v$ has been distinguished from. This set grows without bound as $k \to
\infty$. The negation is never complete: there is always a $G_{k+1}$
containing a new node not yet in the NOT-sequence of $v$ in $G_k$. But at
every finite $k$, $v$ is individuated: $\sigma_{G_k}(v) \geq \bmin(G_k) > 0$.
The individuation does not require completing the negation. It requires only
that the current graph has positive edge weights, which it always does. \qed
\end{proof}

\begin{remark}[The Asymmetry Between Finite and Infinite]
If the universe is infinite and $v$ is finite (bounded boundary cost), then
comparing $v$ with the universe is an infinite process --- but $v$'s identity
is not in doubt at any finite stage. The comparison cannot be finished. The
identity does not depend on finishing it. This is the graph-theoretic resolution
of the apparent paradox: infinite negation does not prevent finite identity.
The floor $\bmin^*(v) > 0$ is the identity, and it is available at every
finite stage of an infinite process.
\end{remark}
...
\subsection{Mass as the Intrinsic Floor}

The mass spectrometer is the clearest physical instantiation of the framework,
because it makes the intrinsic floor observable directly.

\begin{theorem}[Mass Is the Intrinsic Floor]
\label{thm:mass_as_floor}
In any mass spectrometric measurement, the mass $m$ of an ion species $v$
is the intrinsic floor $\bmin^*(v)$: the unique property of $v$ that is
invariant across all instrument generations, all matrices, all sample
preparation histories, and all resolving powers. Every other measurable
property of $v$ --- fragmentation pattern, retention time, isotope ratio
at a given resolution, ionisation efficiency --- is a truth cell edge at
some finite depth $n$, changeable by reshuffling. Mass alone is the limit
of the truth cell weight as the graph expands without bound.
\end{theorem}

\begin{proof}
We establish invariance and uniqueness separately.

\textbf{Invariance of mass.} The mass $m$ of an ion species is determined
by its nuclear and electronic structure. In the graph model, the nuclear
structure of $v$ is encoded in the edges incident to $v$ in the ground-state
graph $G_0$ (the graph containing only the elements, before any analytical
instrument is introduced). Every instrument generation adds new nodes to
$\Nbd(v) \cap \Nbd(\med)$ --- new compounds distinguishable from $v$ at the
new resolving power --- but does not change the edges of $v$ in $G_0$.
The minimum edge weight incident to $v$ in $G_0$ is $\bmin(v, G_0)$, which
is the mass-equivalent separation cost between $v$ and its nearest-mass
neighbour in the initial graph. This weight is unchanged by adding new
neighbours: the existing edge $(v, u_{\mathrm{nearest}})$ remains in all
$G_k$. Hence $\bmin^*(v) = \bmin(v, G_0)$, the mass floor of $v$ in the
initial graph. Mass is invariant.

\textbf{Uniqueness of mass as invariant.} By the Invariance Theorem
(Theorem~\ref{thm:invariance}(ii)), $\bmin^*(v)$ is the only quantity
associated with $v$ that does not change as the graph expands. Every
other analytical property of $v$ is a truth cell edge at some depth:
the fragmentation pattern changes with collision energy (a reshuffling
of the graph at depth $n_{\mathrm{frag}}$); the retention time changes
with column and gradient (a reshuffling at depth $n_{\mathrm{chrom}}$);
the isotope ratio at a given resolving power changes when a new
isobaric compound is added to the graph. Mass alone survives all
reshufficings unchanged. \qed
\end{proof}

\begin{corollary}[The Scale Suffices Iff Identity Is a Point]
\label{cor:scale}
If the identity of an ion species $v$ were a point --- a single exact real
number fully knowable at finite resolving power --- then a sufficiently
precise balance (a scale) would suffice to identify $v$ permanently. No
further instrument would be needed. The existence of mass spectrometry, NMR,
ion mobility spectrometry, and every subsequent generation of higher-resolution
analytical instrument is empirical proof that the identity of $v$ is not a
point. Each instrument generation adds new nodes to the graph, revealing new
structure at the same nominal mass --- new isobaric species, new isotopologue
patterns, new conformational populations. Each generation produces a finer
truth cell, not a point. The identity of $v$ is its intrinsic floor
$\bmin^*(v)$: the region, not the point.
\end{corollary}

\begin{corollary}[All Analytical Methods Reduce the Same Unknowable]
\label{cor:all_methods}
Mass spectrometry, NMR, X-ray crystallography, ion mobility spectrometry,
infrared spectroscopy: each is a graph expansion in a different dimension
of the state space. Each adds new NOT-relations between $v$ and new
neighbours in a new observational axis. None of them approaches the
point-identity of $v$. All of them reduce, by different amounts and along
different axes, the same quantity: the unknowable residue of $v$ relative
to the current graph. The identity of $v$ is the structured totality of
what remains unknowable after all methods have been applied at all
resolutions --- the intrinsic floor $\bmin^*(v)$, not the label ``Lithium.''
\end{corollary}
...
\begin{remark}[The Sample Confirmation Regress]
\label{rem:confirmation_regress}
To trust a mass reading, one must trust that what was inserted into the
instrument is what one claims. To trust the insertion, one must trust a
prior measurement. In graph terms: the node $v_{\mathrm{sample}}$ is itself
individuated only within the graph of all prior preparation steps. Each
preparation step is a reshuffling at some depth. The claim ``this is pure
Lithium'' is itself a truth cell claim at the depth of the finest prior
measurement. The reading does not measure the sample. It measures the
entire experimental history --- the composition of all reshufficings applied
to the material since its last uncontrolled state. Mass is the only thing
that survives intact through the entire history. It is the only thing that
cannot be reshuffled away. It is the floor.
\end{remark}

\begin{proposition}[The Mass Edge and the Analytical Edge Are Distinct]
\label{prop:mass_vs_analytical}
Let $v$ be an ion species node with medium $\med$ in a mass spectrometric
graph $G$. The graph $G$ contains two types of edges incident to $v$:
\begin{enumerate}[label=(\roman*)]
  \item The \emph{mass edge} $(\med, v)$ of weight $w(\med, v) = \bmin^*(v)$:
  the medium-to-$v$ contact, representing the ion's intrinsic mass.
  \item \emph{Analytical edges} $(v, u_k)$ for nodes $u_k$ representing
  fragmentation products, adducts, or co-eluting species: these encode the
  ion's behaviour under the current instrument configuration.
\end{enumerate}
The mass edge is invariant: $w(\med, v)$ is the same in every graph expansion
$G_0 \subset G_1 \subset \cdots$, because the medium is unchanged under
reshuffling (Definition~\ref{def:reshuffling}) and nuclear structure does not
change with instrument generation. The analytical edges are not invariant:
each instrument generation is a reshuffling that replaces or reweights the
$(v, u_k)$ edges without touching $(\med, v)$.

Therefore the identity of $v$ is carried by the mass edge alone. The minimum
edge incident to $v$ (the node floor) is not necessarily the mass edge --- it
is the lightest edge incident to $v$, which may be an analytical edge lighter
than the mass edge. Tracking the node floor conflates the invariant with the
changeable. The invariant quantity is $w(\med, v)$ specifically.
\end{proposition}

\begin{proof}
By Definition~\ref{def:reshuffling}, a reshuffling preserves medium adjacency:
the medium $\med$ remains adjacent to all nodes. The graph-expansion model of
instrument generations (Theorem~\ref{thm:instrument_generation}) adds new
nodes and edges without removing the existing medium edge $(\med, v)$, so
$w(\med, v) = \bmin^*(v)$ is preserved across all $G_k$. By contrast, each
new generation adds new analytical neighbours $u_k'$ to $v$ with new edge
weights $w(v, u_k')$, which may be lighter than the mass edge. The mass edge
persists; the analytical edge set changes.

The minimum incident edge $\min\{w(e) : e \ni v\}$ therefore potentially
tracks an analytical edge (if $w(v, u_k) < w(\med, v)$ for some $k$) rather
than the mass. The intrinsic floor $\bmin^*(v) = w(\med, v)$ is distinct from
the node minimum, and only the former is the identity of $v$. \qed
\end{proof}

\begin{remark}[The NOT-Sequence Has a Common Lower Bound]
\label{rem:not_floor}
Proposition~\ref{prop:not_sequence} establishes that the NOT-sequence of
$v$ --- the growing set of cuts $C^*_{G_k}(v)$ --- has all cut weights
bounded below by $\bmin(G_0) > 0$. This means the NOT-sequence does not
converge to zero even as the universe grows: at every finite stage of the
negation process, the minimum cut has positive weight. The sequence of
NOT-relations is monotone in size (new NOT-relations are added but never
removed) and bounded below in weight. The floor $\bmin^*(v) > 0$ is the
common lower bound of the entire NOT-sequence, not merely a limit.
\end{remark}

\begin{remark}[Count Interval Holds Throughout Transition, Not Only at Endpoints]
\label{rem:count_interior}
Theorem~\ref{thm:count_interval} states that the count lies in $[n, n+1)$
during any transition. The validation for this paper confirmed a stronger
form: the count is in $(n, n+1)$ --- strictly between the integers --- for
every instant interior to the transition. The integer values $n$ and $n+1$
are achieved only at the completed states (before and after the transition,
respectively), not during it. The interior of the transition is a strictly
open interval, not a half-open one. The finest honest partition of the count
during a transition is therefore $\{n, n+1\}$: the two bounding integers,
neither of which is the count at any interior instant.
\end{remark}

\begin{remark}[The Same $n$ Generations Admit $T(n,d)$ Distinct Termination Structures]
\label{rem:composition_inflation}
Proposition~\ref{prop:incompletable} establishes that the NOT-sequence of a
node $v$ grows strictly with each graph expansion and never completes. A
stronger statement governs the \emph{combinatorial structure} of those
expansions. Let $n$ be a fixed number of instrument generations and $d$ be
the number of independent weight dimensions on the edges of $G$.

The $n$ generation steps can themselves be \emph{composed}: any ordered
partition $(c_1, c_2, \ldots, c_k)$ with $c_1 + \cdots + c_k = n$ and each
$c_i \geq 1$ describes a different grouping of the $n$ steps into $k$
sequential blocks, where each block refines one of the $d$ weight dimensions.
The total number of such labeled compositions is
\[
  T(n, d) = d \cdot (d+1)^{n-1}.
\]
This is not the count of NOT-sequence elements (which is $n$) but the count
of \emph{distinct termination structures} reachable by $n$ generation steps:
different orderings and groupings of the same $n$ steps produce different
paths through the truth-cell space, each arriving at a different minimum cut
configuration in $G_n$.

The consequence for individuation is: the same physical item (the node $v$
after $n$ measurement generations) does not have a single termination point
in the truth-cell space. It has $T(n, d)$ distinct termination structures,
one for each labeled composition of the $n$ generation steps. Which
termination structure is actualised depends on the order and grouping of the
analytical refinements, not only on their total count. The contact floor
$\bmin^*(v) > 0$ bounds every one of these $T(n,d)$ structures from below:
no compositional ordering of the generations drives the mass edge weight to
zero.
\end{remark}
...
\subsection{Finitude and Invariance Are the Same Condition}

The Invariance Theorem reveals that the two intuitive requirements for a thing
to exist --- that it be finite, and that its identity persist through an
incompletable negation --- are not two separate requirements. They are both
the statement $\bmin^*(v) > 0$.

Finitude says: the boundary of $v$ has a positive minimum cost. Infinitely
extended things have no boundary distinguishable from the medium; they are
the medium. The condition for $v$ to be a finite thing is that it costs
something positive to separate it from everything else.

Invariance says: whatever identifies $v$ must be stable as the set of NOT-things
grows. If the universe is infinite, the NOT-process never terminates --- but the
thing is identified at every finite stage by the same invariant. The condition
for this invariant to exist is that something about $v$ does not change as the
graph grows. By Corollary~\ref{cor:identity_is_floor}, the only such thing is
$\bmin^*(v)$.

Both conditions reduce to: $\bmin^*(v) > 0$. The floor is not just the minimum
cost of a boundary. It is the definition of what a thing is.
...
\item \textbf{The intrinsic floor exists and is positive}
  (Theorem~\ref{thm:invariance}): for any node $v$ in a graph sequence
  expanding without bound, the limit $\bmin^*(v) = \lim_k \bmin(v, G_k)$
  exists, is strictly positive, and is independent of the universe's size.
  The two conditions for $v$ to exist as a distinct thing --- finitude and
  invariance under an incompletable negation --- are both the single
  condition $\bmin^*(v) > 0$.

\item \textbf{Identity is the floor} (Corollary~\ref{cor:identity_is_floor}):
  the identity of $v$ is $\bmin^*(v)$. Not its label. Not its neighbourhood.
  Not its truth cell. The label is a convenience. The neighbourhood changes
  with every reshuffling. The truth cell changes with every instrument
  generation. Only the intrinsic floor is stable.

\item \textbf{Mass is the intrinsic floor in mass spectrometry}
  (Theorem~\ref{thm:mass_as_floor}): mass is the unique analytical property
  of an ion invariant across all instrument generations, matrices, and sample
  histories. All other properties are truth cell edges at finite depth,
  changeable by reshuffling. The existence of successive generations of
  higher-resolution instruments is empirical proof that the identity of an
  ion is not a point: each generation reveals new structure at the same
  nominal mass, deepening the truth cell without collapsing it.
...
itself is what the node is. If a scale sufficed, there would be no mass
spectrometer. The spectrometer exists because the floor is not a point.
