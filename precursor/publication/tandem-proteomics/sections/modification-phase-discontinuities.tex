...
Fragment phase $\Phi(b_i)$ is not directly measured but reconstructed from intensity relationships:

\begin{theorem}[Phase Reconstruction from Intensities]
\label{thm:phase_reconstruction}
For fragments forming sequential ladder $\{b_1, b_2, \ldots, b_L\}$, phases can be reconstructed through:
\begin{equation}
\Phi(b_k) = \Phi_0 + \sum_{i=1}^{k-1} \omega_0\sqrt{\frac{\Delta m_i}{m_{\text{AA,avg}}}} + \sum_{i=1}^{k-1} \delta\phi_i
\end{equation}
where $\Phi_0$ is arbitrary reference phase, $\Delta m_i = m_{b_{i+1}} - m_{b_i}$ is mass difference, and $\delta\phi_i$ is phase correction from intensity pattern:
\begin{equation}
...
\begin{proof}
Intensity deviations from mean encode phase information through the intensity-phase relation $I_i \propto \exp(-|E_i|/\langle E \rangle)$ combined with phase-dependent edge formation. High-intensity fragments (above mean) indicate negative phase correction (fewer edges formed), low-intensity fragments indicate positive phase correction (more edges formed).

The arctangent transformation maps intensity ratios to phase corrections in range $(-\pi/2, \pi/2)$, consistent with phase-lock theory where phase differences $< \pi$ dominate.
...
$Z$-score represents discontinuity significance relative to unmodified backbone fluctuations ($\sigma_{\text{baseline}} = 0.41$ rad). PTMs with $Z > 3$ are reliably detectable, while small modifications (deamidation, $Z = 1.2$) require additional evidence.
...
Complexity: $O(L)$ for phase computation and discontinuity scanning, versus $O(L \cdot N_{\text{sites}})$ for single-PTM site enumeration or $O(L^k)$ for $k$ PTMs.
...
Phase discontinuity significance testing controls false positive rate:
...
For multiple testing correction (testing $L-1$ sites), apply Bonferroni:
\begin{equation}
...
FDR control maintained across PTM counts, validating statistical framework.
...
Given observed discontinuity $|\Delta\Phi_{\text{obs}}|$, the modification mass is:
\begin{equation}
...
Some PTMs exhibit "crosstalk": one modification influences another's phase signature. For example, phosphorylation at S$_{k}$ affects phase signature of nearby phosphorylation at S$_{k+2}$ through extended phase coupling.
...
\end{equation}
where $\lambda_{\phi} = 2\pi c/\omega_0 = 4.8 \pm 1.1$ Å is phase wavelength and $r_{\text{AA}} = 3.8$ Å is amino acid spacing along backbone.
\end{equation}
\end{proposition}
...
This handles challenging cases like adjacent phosphorylations common in kinase motifs (e.g., S-X-X-S or S-S motifs).
...
\begin{itemize}
\item Synthesize peptides with known modification sites
\item Measure MS/MS spectra
\item Verify phase discontinuities match predicted positions
...
\begin{itemize}
\item Derivatize PTMs (e.g., $\beta$-elimination of phosphoserine)
\item Phase discontinuity magnitude should change by derivatization mass
...
Combined AND scoring (require agreement) achieves lowest FDR (0.6\%) and ambiguity (8.9\%) at modest sensitivity cost (83.1\%). Combined OR scoring (accept either) achieves highest sensitivity (92.9\%) at controlled FDR (5.4\%).
...
Phase discontinuity approach handles glycosylation through:

\begin{enumerate}
\item \textbf{Large discontinuities}: Glycosylation creates $|\Delta\Phi| = 3.8 \pm 0.7$ rad ($Z = 6.2$), highly significant
