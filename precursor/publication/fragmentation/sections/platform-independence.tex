...
Mass range: 150-1200 Da. All compounds measured in triplicate on each platform over 3 days.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures/intensity_platform_variation.pdf}
\caption{\textbf{Raw intensity platform dependence.} (A) Scatter plot of fragment intensities for palmitic acid (m/z 256.2) measured on Waters (x-axis) versus Thermo (y-axis). Pearson $r = 0.48$, indicating modest correlation. (B) Intensity ratio distribution: Thermo/Waters intensity ratios span 0.2-5.1× (5th-95th percentile), with median 1.3×. (C) Platform bias varies by fragment type: low-mass fragments ($m/z < 150$) show 2.3× Waters bias, high-mass fragments ($m/z > 400$) show 1.8× Thermo bias. (D) Collision energy dependence: intensity ratios vary 3.2-fold across 20-50 eV range, preventing cross-platform calibration without energy normalization.}
\label{fig:intensity_variation}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures/universal_library.pdf}
\caption{\textbf{Platform-invariant spectral library.} (A) Library structure: 247 compounds, each measured on 2 platforms, represented as single entry using mean S-entropy coordinates. (B) Cross-platform library matching accuracy: S-entropy library (blue) achieves 91.4\% rank-1 accuracy regardless of query platform, while intensity library (orange) drops from 89.3\% (same platform) to 62.4\% (cross-platform). (C) Library size scaling: S-entropy approach requires 1 entry per compound, intensity approach requires $N_{\text{platforms}}$ entries per compound. For 10,000 compounds across 5 platforms, S-entropy library is 5× smaller. (D) Update efficiency: adding new platform to S-entropy library requires $O(1)$ work per compound (validation only), intensity library requires $O(N_{\text{compounds}})$ remeasurements.}
\label{fig:universal_library}
\end{figure}

Universal library advantages:
\begin{itemize}
\item \textbf{Size reduction}: Factor of $N_{\text{platforms}}$ smaller (5-10× for typical applications)
...
Mean CV $= 3.7\%$ across 20-40 eV range indicates collision energy affects fragmentation extent (more fragments at higher energy) but preserves topological relationships. Phase features show highest energy dependence (CV $= 6.3\%$) as expected from energy-dependent decoherence rates.

For practical applications, collision energy normalization (20 eV per 100 Da precursor mass) reduces CV to $< 2.5\%$ across all features.

\subsection{Ion Source Independence}

S-entropy features remain stable across different ionization methods:
...
Ionization method primarily affects precursor ion formation, not fragmentation topology. CV $= 3.5\%$ across ESI and APCI confirms categorical states are independent of ionization mechanism.
...
All tests fail to reject platform equivalence hypothesis at $\alpha = 0.05$ level, providing statistical evidence that Waters and Thermo platforms produce equivalent S-entropy feature distributions.
...
S-entropy achieves lowest CV (1.4\%) and highest transfer accuracy (81.9\%) without requiring reference standards, batch labels, or calibration measurements—platform independence is intrinsic, not empirically achieved.
...
All platforms maintain $D < 0.15$ (threshold for valid categorical states), with low cross-platform CV $= 16.2\%$. This confirms hardware grounding operates consistently across instrument types, providing universal quality metric.

Incorrect molecular assignments (wrong compound ID, contamination) exhibit $D > 0.35$ regardless of platform, enabling automatic error detection without manual review.
...
\begin{enumerate}
\item \textbf{Library construction}: Measure each compound on any single platform, compute S-entropy coordinates
\item \textbf{Query analysis}: Extract query spectrum S-entropy coordinates, match against library using Euclidean distance
\item \textbf{Threshold selection}: Distance $< 0.27$ indicates same molecule (95th percentile cross-platform distance)
\item \textbf{Rank scoring}: Report top 5 matches with distances, confidence from separation from next-best match
