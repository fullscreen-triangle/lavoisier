...
\textbf{Challenge 1: Platform Dependence.} Spectra acquired on different instrument types (quadrupole time-of-flight, Orbitrap, ion trap) exhibit systematic intensity variations preventing cross-platform spectral library matching and model transfer \cite{stein2012mass,horai2010massbank}. The same molecule on different instruments produces recognizable but quantitatively distinct fragmentation patterns.

\textbf{Challenge 2: Intensity Prediction.} No first-principles theory connects molecular structure to fragment intensities. Empirical models achieve limited success through machine learning on large training sets \cite{wei2019rapid,duhrkop2019sirius}, but lack mechanistic insight and fail for novel chemical classes.
...
Categorical theory introduces a different perspective: the precursor ion occupies an initial categorical state $\mathcal{C}_0$ characterized by its internal phase-lock network. Upon collision activation, the molecule does not randomly break bonds but progresses through a sequence of categorical states:
\begin{equation}
...
\begin{enumerate}
\item Its molecular structure (atoms, bonds, charge location)
\item Residual phase correlations with other fragments
\item Position in the irreversible categorical sequence
\end{enumerate}

Critically, fragments from the same precursor maintain phase correlations analogous to the residual A-B edges observed after gas mixing in the Gibbs' paradox resolution \cite{sachikonye2024gibbs}. These correlations explain why:
\begin{itemize}
...
Fragment intensity does not reflect statistical population but oscillatory termination probability. Following the oscillatory entropy formulation:
\begin{equation}
...
\begin{enumerate}
\item \textbf{Topological fragmentation theory}: Fragment intensity arises from phase-lock network density via Eq.~\eqref{eq:intensity_formula}, providing first-principles connection between structure and spectrum.
...
\item \textbf{Neutral loss prediction}: Phase memory analysis achieves 94.3\% accuracy on water/CO$_2$/ammonia loss predictions with measurable coherence time $\tau_{\phi} = 23.4 \pm 6.7$ ns.

\item \textbf{Fragment correlation structure}: Residual phase correlations decay as $\rho_{ij}(t) = \rho_0 \exp(-t/\tau_{\phi})$, confirming phase-lock hypothesis and enabling correlation-based structure elucidation.
...
% Import sections
\input{sections/fragment-states-correlations}
\input{sections/s-entropy-feature-extraction}
...
\textbf{(1) Fragmentation is Irreversible.} Each categorical state completion is irreversible per Axiom of Categorical Irreversibility. Once fragment $i$ occupies categorical state $\mathcal{C}_i$, the system cannot return to $\mathcal{C}_i$. This explains why fragmentation spectra are reproducible: the categorical sequence is deterministic despite stochastic collision dynamics.
