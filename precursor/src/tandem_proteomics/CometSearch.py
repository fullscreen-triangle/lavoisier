...
class CometParams(object):
    ...
    def load_attributes(self):
        ...
        if self.which_one == 'comet':
            ...
            self.num_enzyme_termini = int(
                self.params_dict['num_enzyme_termini'])  # need to set this for filtering protein matches
...
class CometSearch:
    ...
    def __init__(self, ms2_folder: str, pep_mass_tol, pep_mass_units, mass_type_par, mass_type_frag, frag_ion_type,
                 enzyme,
                 database, cleavage):
        self.ms2_folder = ms2_folder
        self.pep_mass_tol = pep_mass_tol
        self.pep_mass_units = pep_mass_units
        self.mass_type_par = mass_type_par
        self.mass_type_frag = mass_type_frag
        self.frag_ion_type = frag_ion_type
        self.enzyme = enzyme
    ...
    def save_settings(self):
        user_selected_params = {
            ...
            'peptide_mass_tolerance': self.pep_mass_tol,
            'peptide_mass_units': self.pep_mass_units,
            'mass_type_parent': self.mass_type_par,
            'mass_type_fragment': self.mass_type_frag,
            'use_B_ions': '1',
            'use_Y_ions': '1',
        ...
        if self.frag_ion_type == 0:
            user_selected_params['fragment_bin_tol'] = ('1.0005', '# binning to use on fragment ions')
            user_selected_params['fragment_bin_offset'] = (
                '0.4', '# offset position to start the binning (0.0 to 1.0)')
            user_selected_params['theoretical_fragment_ions'] = ('1', '# 0=use flanking peaks, 1=M peak only')
            user_selected_params['spectrum_batch_size'] = (
                '0', '# max. # of spectra to search at a time; 0 to search the entire scan range in one loop')
        else:
            user_selected_params['fragment_bin_tol'] = ('0.02', '# binning to use on fragment ions')
            user_selected_params['fragment_bin_offset'] = (
                '0.0', '# offset position to start the binning (0.0 to 1.0)')
            user_selected_params['theoretical_fragment_ions'] = ('0', '# 0=use flanking peaks, 1=M peak only')
            user_selected_params['spectrum_batch_size'] = (
                '20000', '# max. # of spectra to search at a time; 0 to search the entire scan range in one loop')
        if self.cleavage == 1:
            user_selected_params['num_enzyme_termini'] = (
                '1', '# 1 (semi-digested), 2 (fully digested, default), 8 C-term unspecific , 9 N-term unspecific')
                ...
                if key in user_selected_params:  # come back and do string formatting
                    if key == 'peptide_mass_tolerance':
                        ...
                        f.write('%s = %s%s%s\n' % (key, user_selected_params[key][0],
                                                   (36 - len(key) - len(user_selected_params[key][0])) * ' ',
                                                   user_selected_params[key][1]))
