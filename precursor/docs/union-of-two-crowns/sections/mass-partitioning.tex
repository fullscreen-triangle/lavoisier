...
\begin{itemize}
    ...
    \item Mass $m$ as partition occupation (Section 4.5)
    \item Classical mechanics from partition dynamics (Section 4.5)
    \item Thermodynamic laws from categorical structure (Section 5)
    \item Electromagnetic laws from charge coupling (Section 5)
    \item Transport coefficients from partition lag (Section 7)
\end{itemize}

The question now: \textit{How do we measure these partition coordinates?}

Given a gas of ions, each occupying some state $(n_i, \ell_i, m_i, s_i)$ with mass $m_i$ and charge $q_i$, we cannot directly observe which partition cell each ion occupies. We can only apply forces and measure responses.

This section derives the necessary and sufficient conditions for extracting partition coordinates from charged particle dynamics. We prove that these conditions uniquely determine a device architecture—what we recognize as a mass spectrometer.
...
\subsection{Summary: MS as Partition Coordinate Extraction}

We have derived mass spectrometry from first principles:
...
\begin{enumerate}
    \item Ion source (EI, ESI, MALDI)
    \item Mass analyzer (TOF, quadrupole, ion trap, Orbitrap, FT-ICR)
    \item Detector (electron multiplier, Faraday cup, image current)
\end{enumerate}

\textbf{Analyzer principles:}
\begin{itemize}
    \item TOF: Temporal separation by $t \propto \sqrt{m/q}$
    \item Quadrupole: Stability filtering by Mathieu parameters
    \item Ion trap: Secular frequency $\omega_{\text{sec}} \propto q/m$
    \item Orbitrap: Axial frequency $\omega \propto \sqrt{q/m}$
    \item FT-ICR: Cyclotron frequency $\omega_c = qB/m$
\end{itemize}

\textbf{Fragmentation:}
\begin{itemize}
    \item CID: Energy transfer $E_{\text{CM}} = E_{\text{lab}} m_{\text{gas}}/(m_{\text{ion}} + m_{\text{gas}})$
    \item MS/MS: Sequential analysis for structural information
\end{itemize}

All from:
\begin{equation}
\text{Partition coordinates } (n, \ell, m, s) \to \text{Mass } m \to \text{Trajectories } \mathbf{r}(t) \to \text{Measurement}
\end{equation}

The mass spectrometer is not an arbitrary analytical instrument—it is the unique device architecture that satisfies the necessary and sufficient conditions for extracting partition coordinates from charged particle ensembles.
...
Mass spectrometry is partition coordinate measurement made physical.
